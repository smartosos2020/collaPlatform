...
public class JdbcKnowledgeBaseSpaceRepository implements KnowledgeBaseSpaceRepository {
        ...
        return jdbcTemplate.query(
            """
                select k.id, k.name, k.code, k.description, k.icon, k.cover_url, k.status, k.visibility,
                       k.root_document_id, k.home_document_id, k.owner_id,
    ...
    @Override
    public Optional<KnowledgeBaseSpaceSummary> findSpaceByRootDocumentId(UUID workspaceId, UUID rootDocumentId) {
        try {
            return Optional.ofNullable(jdbcTemplate.queryForObject(
                """
                    select k.id, k.name, k.code, k.description, k.icon, k.cover_url, k.status, k.visibility,
    ...
    @Override
    public Optional<KnowledgeBaseSpaceSummary> findSpaceByDocumentId(UUID workspaceId, UUID documentId) {
        try {
            return Optional.ofNullable(jdbcTemplate.queryForObject(
                """
    ...
    @Override
    public Optional<UUID> findDisabledRootForDocument(UUID workspaceId, UUID documentId) {
        try {
            return Optional.ofNullable(jdbcTemplate.queryForObject(
                """
                    with recursive ancestors as (
    ...
    @Override
    public List<UUID> listSubscribedDocumentIds(UUID workspaceId, UUID subscriberId, UUID rootDocumentId, int limit) {
    ...
    @Override
    public List<UUID> listSubscriberIdsForDocument(UUID workspaceId, UUID documentId) {
        return jdbcTemplate.queryForList(
            """
                ...
                kb as (
                    select k.id
                    from knowledge_base_spaces k
                    join ancestors a on a.id = k.root_document_id
                ...
                where ks.workspace_id = ?
                  ...
                  and (
                      ...
                      or (ks.target_type = 'document' and ks.target_id in (select id from ancestors))
                  )
                """,
            UUID.class,
            workspaceId,
    ...
    private KnowledgeBaseSpaceSummary mapSummary(ResultSet rs, int rowNum) throws SQLException {
        return new KnowledgeBaseSpaceSummary(
            ...
            rs.getObject("root_document_id", UUID.class),
            rs.getObject("home_document_id", UUID.class),
