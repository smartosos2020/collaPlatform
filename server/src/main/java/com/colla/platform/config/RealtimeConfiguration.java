...
public class RealtimeConfiguration {
    @Bean
    RealtimeRedisAvailability realtimeRedisAvailability(
        ObjectProvider<WebSocketSessionRegistry> sessionRegistry
    ) {
        return new RealtimeRedisAvailability(() -> {
            WebSocketSessionRegistry registry = sessionRegistry.getIfAvailable();
            if (registry != null) {
