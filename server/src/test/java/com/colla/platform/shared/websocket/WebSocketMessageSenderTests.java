...
class WebSocketMessageSenderTests {
    ...
    @Test
    void slowClientDoesNotDelayHealthyClient() throws Exception {
        WebSocketSessionRegistry registry = mock(WebSocketSessionRegistry.class);
        @SuppressWarnings("unchecked")
        ObjectProvider<WebSocketSessionRegistry> provider = mock(ObjectProvider.class);
        when(provider.getIfAvailable()).thenReturn(registry);
        ...
        org.mockito.Mockito.doAnswer(ignored -> {
            ...
            assertThat(releaseSlow.await(5, TimeUnit.SECONDS)).isTrue();
            return null;
        }).when(slow).sendMessage(any(TextMessage.class));
        RealtimeSignalEnvelope signal = envelope(Audience.workspace());
        when(registry.workspaceSessions(signal.workspaceId())).thenReturn(Set.of(slow, healthy));
        RealtimeProperties properties = properties(8);
        properties.setSendThreads(2);
        WebSocketMessageSender sender = sender(provider, properties);
        try {
            sender.sendRealtime(signal);

            assertThat(slowStarted.await(5, TimeUnit.SECONDS)).isTrue();
            verify(healthy, timeout(2_000)).sendMessage(any(TextMessage.class));
    ...
    @Test
    void sendFailureOnlyRemovesTheBrokenConnection() throws Exception {
        WebSocketSessionRegistry registry = mock(WebSocketSessionRegistry.class);
        @SuppressWarnings("unchecked")
        ObjectProvider<WebSocketSessionRegistry> provider = mock(ObjectProvider.class);
        when(provider.getIfAvailable()).thenReturn(registry);
        ...
        RealtimeSignalEnvelope signal = envelope(Audience.workspace());
        when(registry.workspaceSessions(signal.workspaceId())).thenReturn(Set.of(broken, healthy));
        ...
        try {
            ...
            verify(broken, timeout(2_000)).close(CloseStatus.SESSION_NOT_RELIABLE);
            verify(registry, timeout(2_000)).unregister(broken);
            verify(healthy, timeout(2_000)).sendMessage(any(TextMessage.class));
            verify(healthy, never()).close(any(CloseStatus.class));
            assertThat(meters.counter("colla.realtime.websocket.send", "outcome", "failed").count())
                .isEqualTo(1);
        } finally {
            sender.shutdown();
        }
    ...
    @Test
    void legacyFrameSwitchBlocksDirectSessionDeliveryAndRecordsAttempt() throws Exception {
        WebSocketSessionRegistry registry = mock(WebSocketSessionRegistry.class);
        @SuppressWarnings("unchecked")
        ObjectProvider<WebSocketSessionRegistry> provider = mock(ObjectProvider.class);
        when(provider.getIfAvailable()).thenReturn(registry);
        WebSocketSession target = openSession("legacy-target");
        UUID userId = UUID.randomUUID();
        UUID workspaceId = UUID.randomUUID();
        when(registry.sessions(userId, workspaceId)).thenReturn(Set.of(target));
        RealtimeProperties properties = properties(8);
        properties.setLegacyFramesEnabled(false);
        SimpleMeterRegistry meters = new SimpleMeterRegistry();
        WebSocketMessageSender sender = new WebSocketMessageSender(
            provider,
            new ObjectMapper().findAndRegisterModules(),
        ...
        try {
            ...
            assertThat(meters.counter(
                "colla.realtime.websocket.legacy.frame",
                "outcome",
                "blocked"
            ).count()).isEqualTo(1);
        } finally {
