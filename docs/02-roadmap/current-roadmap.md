...
| 任务 | 内容 | 验收标准 | 状态 |
| --- | --- | --- | --- |
| KB-PRODUCT-M12-T01 | 汇总 M1-M11 缺陷并完成 P0/P1/P2 处理决定 | P0/P1 清零；必须修复 P2 关闭；延期项有风险接受人 | Pending |
| KB-PRODUCT-M12-T02 | 执行知识数据、块、版本、评论、权限、搜索和对象引用一致性检查 | 无未解释悬空、泄漏、旧兼容写入和不可恢复内容 | Pending |
| KB-PRODUCT-M12-T03 | 执行完整后端测试、package 和 Flyway 空库/存量升级 | 全部通过，迁移版本、备份和回滚证据完整 | Pending |
| KB-PRODUCT-M12-T04 | 执行前端 lint/build、知识库完整浏览器和双用户协同回归 | 用户/管理员、读/评/编/管权限及双节点协同全部通过 | Pending |
| KB-PRODUCT-M12-T05 | 执行备份、恢复、发布和回退演练 | 恢复后块、版本、评论、对象引用和协同快照一致 | Pending |
