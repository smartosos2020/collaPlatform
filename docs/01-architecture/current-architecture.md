...
- 数据库：单个 PostgreSQL schema，通过 Flyway V001-V060 演进。
...
最近一次已归档路线级全量验证为 KB-NAME-M11：后端 60/60，通过当时的空库迁移、后端 package、前端 build、安全审计、命名和文档门禁；PROJECT-PLATFORM-S01-M4 重新执行过 V001-V055 `route-final`。当前路线级全量验证基线为 PROJECT-PLATFORM-S02-M5 `route-final`（V001-V060 空库迁移、完整后端测试、后端 package、前端 lint/build、安全与文档门禁），其结果记录在 M5 执行报告和本地质量门禁证据中。
...
S01 已确定未来使用 ProjectSpace + versioned WorkItemType + WorkItem、规范 JSONB + capability typed projection、显式 legacy ID map、canonical-only write cutover 和状态/节点双运行时。S02-M1 至 M4 已落地空间模型、API、成员治理、双 UI 和 legacy project -> space/member 映射执行与兼容入口；project/issue 迁移与旧写关闭属于 S07，不能因为新空间入口上线而提前改变 legacy 写事实。
