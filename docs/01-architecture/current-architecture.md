...
- 数据库：单个 PostgreSQL schema，通过 Flyway V001-V063 演进。
...
最近一次已归档路线级全量验证为 KB-NAME-M11：后端 60/60，通过当时的空库迁移、后端 package、前端 build、安全审计、命名和文档门禁；PROJECT-PLATFORM-S01-M4 重新执行过 V001-V055 `route-final`。当前路线级全量验证基线为 PROJECT-PLATFORM-S03-M5 `route-final`（V001-V063 空库迁移、完整后端测试、后端 package、前端 lint/build、协作测试、安全与文档门禁），其结果记录在 S03-M5 执行报告和本地质量门禁证据中。
...
- S03 只交付类型定义和首个不可变骨架版本：当前没有 `project_work_items` 表、工作项实例 API、动态字段值、布局、流程或完整草稿发布流水线。S04 只承接字段定义、选项、规则与查询投影合同；S06 承接新配置版本发布，S07 承接显式绑定 `type_version_id` 的统一实例。
...
以上是当前实现边界；动态字段、完整配置发布、统一 WorkItem 实例、流程版本、统一权限解释和可配置视图仍是目标架构能力，不得写成已交付。
