...
| `document` | doc | `/knowledge-bases/{spaceId}/items/{id}`；`/docs/{id}` 仅兼容 | `colla://document/{id}` | 历史对象类型；用户侧展示为“知识内容”，用于知识内容卡片、搜索、通知、IM 链接和旧 deep link |
...
- `issue`
- `document`，历史 objectType；用户侧展示为“知识内容”，默认排除已归档内容节点
- `base`
- `base_table`
- `base_record`
- `message`
...
| `source` | 权限来源，例如 `project_members`、`resource_permissions(document)`、`base_members`、`approval_participants`；`document_permissions` 仅是旧数据兼容来源 |
| `reason` | 可展示给用户的解释文案 |
...
`DocumentPlatformObjectResolver` 解析历史 `document` objectType，并按知识内容权限返回摘要：

- 可访问内容返回标题、知识库主 Web path、`colla://document/{id}` 和知识库路径 metadata；有知识库上下文时主跳转是 `/knowledge-bases/{spaceId}/items/{id}`，`/docs/{id}` 仅作兼容 fallback。
- 已归档内容节点的 `status` 返回 `archived`，普通内容节点返回 `active`。
- `metadata` 当前包含 `docType`、`archived`、知识库 ID、知识库名称和知识库路径，供 IM 卡片、搜索结果和后续知识内容嵌入块复用。
- `/docs/{id}` 当前只作为历史 deep link、分享链接和兼容编辑路由；用户侧主入口是 `/knowledge-bases` 和 `/knowledge-bases/{spaceId}/items/{id}`。
...
知识内容块当前通过 `embedSummary` 复用平台对象摘要；`docs` 前端和 `/api/docs` 后端只承担编辑器底座与旧链接兼容：
