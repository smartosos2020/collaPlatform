...
current_stage: PROJECT-PLATFORM-S06
...
`PROJECT-PLATFORM-S05` 已完成并归档，`PROJECT-PLATFORM-S06` 是唯一活动 Stage。S05 已交付 create/detail 布局图、稳定节点、条件显示、字段访问策略、配置预览、六身份最小披露和共享渲染边界，但没有创建规范 WorkItem、动态字段值或发布版本。S06 负责建立唯一可变 `ConfigurationDraft`、自包含不可变发布快照、原子发布、差异/回滚和模板 lineage；不得提前创建 S07 的工作项实例或运行时。`PLATFORM-SCALE-S01-S04` 已建立模块边界门禁、公共合同、双 API、可靠双 Worker、双 Event Gateway、统一客户端校准和唯一知识协同协议；`PLATFORM-SCALE-S05-M1` 只建立容量验证环境、确定性种子和四类加载器，M2-M5 继续 Deferred，不构成生产容量或基础设施 HA 承诺。
...
| PROJECT-PLATFORM-S06 | 配置草稿、发布、版本和模板复用 | S03-S05 | Active | 配置发布事务、版本升级、回滚和复用/解绑 |
