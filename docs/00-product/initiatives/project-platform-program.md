...
current_stage: PROJECT-PLATFORM-S02
...
| PROJECT-PLATFORM-S02 | 项目空间、成员和空间治理 | S01 | Active | 空间生命周期、成员角色、可见性和治理 API |
...
| PROJECT-PLATFORM-S03-M2 | 类型管理 API 与空间配置 UI | 可创建、复制、排序、停用和恢复类型 |
| PROJECT-PLATFORM-S03-M3 | 研发预置类型映射 | 项目、需求、任务、缺陷、版本、迭代成为配置 |
| PROJECT-PLATFORM-S03-M4 | 类型约束与兼容测试 | 删除/停用不破坏历史实例和引用 |
...
| PROJECT-PLATFORM-S04-M4 | 字段管理 UI、查询投影和性能验证 | 动态字段可筛选、排序且达到预算 |
