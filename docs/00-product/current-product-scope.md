...
| 项目空间 | `/project-spaces`, `/project-spaces/:spaceId`, `/project-spaces/:spaceId/members`, `/project-spaces/:spaceId/settings`, `/project-spaces/:spaceId/types/:typeId?` | 空间列表和最近入口、创建、协作 Shell、成员与邀请治理、角色调整、Owner 转移、空间设置及停用/恢复/归档；owner/admin 可配置工作项类型，执行侧只展示 active 类型摘要 |
