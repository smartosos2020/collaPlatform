...
历史 M40 试运行方案已归档，不能再作为当前准入或数据基线。当前代码已演进到 V047 schema、知识库唯一模型和用户端/管理端双 UI，正式试运行前必须重新建立与当前版本匹配的角色剧本、场景数据、初始化流程、回归清单和 Go/No-Go 标准。

当前只保留以下稳定边界：试运行不得使用过时 M31/M40 重置脚本；真实数据与测试数据必须隔离；回退前必须完成备份和恢复演练；单节点运维按 `docs/05-runbooks/admin-operations.md` 执行。高可用、多节点和集中告警仍不在当前产品化范围内。
...
- 知识库空间壳 URL 使用 `itemId` 表示当前目录项，使用 `view=directory/management` 区分目录内容和轻量空间设置；无效 `itemId` 展示“节点不存在或已无权限”，不会静默跳回首页造成误解。
...
| 试运行 | M40 试运行材料已归档，尚未基于当前 V047 schema 重新建立真实团队试运行准入、初始化和回归基线 |
