...
当前产品开发主线是 `PROJECT-PLATFORM` 项目协作平台长期专项。S01 已完成当前事实审计、领域合同、迁移 spike 和 S02 准入评审；S02-M1 至 M3 已交付项目空间后端、成员治理和双 UI，下一里程碑是 legacy project 到空间/成员的映射与兼容入口。目标能力记录在 `docs/00-product/initiatives/project-platform-program.md`，目标架构记录在 `docs/01-architecture/project-platform-target-architecture.md`；动态类型、字段和统一 WorkItem 仍是后续目标，不能写成当前能力。知识库 `KB-PRODUCT` 工程候选已收口，但 3-5 名真实参与者试用尚未完成，当前按暂停状态归档并等待团队反馈。
...
| 项目空间 | `/project-spaces`, `/project-spaces/:spaceId`, `/project-spaces/:spaceId/members`, `/project-spaces/:spaceId/settings` | 空间列表和最近入口、创建、协作 Shell、成员与邀请治理、角色调整、Owner 转移、空间设置及停用/恢复/归档；设置和成员治理只对空间 owner/admin 展示 |
...
- S02-M1 至 M3 已建立项目空间后端、成员治理和前端闭环：用户“项目”主入口进入 `/project-spaces`，提供最近空间、创建、协作概览、成员治理和空间设置；legacy `/projects` 与 `/issues` 深链继续保留，但不再承担新空间入口。
...
- 现有项目运行入口、API 和数据源仍是 `/projects`、`projects/project_members/issues` 与固定 Java workflow；S02-M1 没有切换 legacy project/issue 业务写，也没有执行 legacy 映射。
- 项目成员与项目群联动。
