...
当前产品开发主线是 `PROJECT-PLATFORM` 项目协作平台长期专项。S01 已完成当前事实审计、领域合同、迁移 spike 和 S02 准入评审；专项在已完成路线归档前暂不激活下一 Stage。目标能力记录在 `docs/00-product/initiatives/project-platform-program.md`，目标架构记录在 `docs/01-architecture/project-platform-target-architecture.md`；这些决策不代表 ProjectSpace、动态类型、字段或统一 WorkItem 已经上线。知识库 `KB-PRODUCT` 工程候选已收口，但 3-5 名真实参与者试用尚未完成，当前按暂停状态归档并等待团队反馈。
...
- S01 只冻结未来演进合同，当前运行入口、API 和数据源仍是 `/projects`、`projects/project_members/issues` 与固定 Java workflow；S02 将先建设空间和成员底座，不会提前切换现有 project/issue 业务写。
- 项目成员与项目群联动。
