...
- S05 布局和字段访问仍是类型内部的待发布配置图。它们不创建 `project_work_items`、动态字段值、上传事实、工作项关系或运行时流程，也不改写 S03 published v1。V078 为布局命令增加不可变响应快照，使相同 request ID 精确重放原响应；S05-M5 已完成最终兼容、回归和 S06/S07 准入收口。
