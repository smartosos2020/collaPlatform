...
| PROJECT-PLATFORM-S21-M8-T07 | core-user | e2e-real-isolated | real | isolated | No | 管理者在工作模型原位管理任务模板，在流程与权限原位选择模板并完成草稿、发布、权限、流程及版本管理，普通成员不可见 |
...
- 修复空间设置入口重复落点：工作模型直接承载任务模板目录与详情；流程与权限使用同一 `typeId` 上下文，直接承载完整配置草稿、发布、权限、流程、模板和版本历史，不再经过四个快捷按钮或任务模板弹窗。
...
| PROJECT-PLATFORM-S21-M8-T07 | 空间管理集中健康、待办、最近入口、危险操作和全部配置 | `ProjectSpaceSettingsPanel` 内嵌模板目录与完整配置草稿；设置 URL 以 `typeId` 共享模板上下文；配置待办、工作项提示与 onboarding 均指向 settings owner | route/onboarding 合同 20/20、backend onboarding 定向测试、frontend lint/build PASS；旧快捷入口组件及跳转合同删除 | real isolated：工作模型与流程权限始终停留 `/settings`，模板跨 Tab/刷新保留；字段/布局往返原位；四按钮、旧嵌套发布 Tab 和选择模板弹窗均不存在 | Done |
...
- Backend：个人工作与活动新增空间范围过滤；类型摘要新增精确 `view/create` 动作；新增 member/guest surface preview 与共享 surface capability assembler；管理员 onboarding 的工作模型、流程发布、自动化与度量步骤统一返回 settings owner 路径。
- Frontend：重组项目空间主/次导航，新增成员工作首页、空间管理首页、成员视图预览、创建影响预览、详情行动摘要与 canonical panel 迁移合同；回归修复新增跨 surface 查询隔离，并将任务模板与完整发布配置原位嵌入工作模型、流程与权限，删除四个快捷按钮、任务模板选择弹窗及其专用跳转合同；未配置状态流程显示中文可行动提示而非空白面板。
...
- 当时修复：集中定义配置入口 URL；任务模板直达目录，字段、表单与页面、发布配置及流程与权限复用配置侧类型查询，多模板时要求管理员明确选择。该方案虽消除了相同落点，但后续真人试用确认入口层本身仍属重复，已由下述原位配置跟进替代。
...
## 2026-07-31 Inline Settings Configuration Follow-up

- 现象：上一轮四类差异化入口仍要求管理员先点按钮、再在弹窗选择任务模板，随后离开设置页；“工作模型”和“流程与权限”本身没有承载所标示的配置内容，操作层级与信息架构仍重复。
- 修复：“工作模型”直接挂载既有任务模板目录与详情；“流程与权限”顶部使用页内任务模板选择器，并直接挂载同一完整配置草稿。`typeId` 作为两个设置 Tab 共享且可刷新的模板上下文；单模板可自动恢复，多模板无上下文时在页内明确选择。
- 清理：删除“任务模板”快捷入口及“字段”“表单与页面”“发布配置”“进入任务模板的流程与权限配置”四按钮，删除选择任务模板弹窗、专用入口组件和跳转合同。任务模板详情中的“配置字段/页面布局”保留为所选模板的上下文操作；字段与布局兼容深链继续可用。
- 入口收敛：管理首页“待发布配置”和工作项未就绪提示均进入 `/settings?panel=flow-access&typeId=...`；正常配置操作不再暴露旧“发布配置”Tab，旧受支持 `/types*` 深链仍保留兼容。
- 入口收敛补充：管理员 onboarding 同步进入工作模型、流程权限、自动化与度量 settings owner；从所选模板进入字段/页面布局后只显示对应编辑器，“返回类型”回到工作模型原位，不再回流旧任务模板/发布配置 Tab。
- 防回归：M8 真实用例断言工作模型原位出现任务模板目录、字段/布局两条上下文往返、选择后仍在 `/settings`、`typeId` 跨 Tab 和刷新保留、无上下文时页内明确选模板、流程权限内草稿/数据权限/唯一流程编辑器可见，以及旧按钮、旧嵌套 Tab 和任务模板弹窗不存在。
...
- Inline-settings follow-up: route/onboarding 合同 20/20、`ProjectSpaceOnboardingServiceTests`、`pnpm --dir web lint`、`pnpm --dir web build` PASS；扩展后的共享运行栈真实动态 fixture 1/1 PASS（41.4s）；独立数据库 real-isolated checkpoint 1/1 PASS（39.7s），证据为 `.local-reports/work-cycle-browser-20260731T115330.log` 与 `.local-reports/quality-gate-20260731T115330.md`，最终 `work:finish` 继续以同一扩展 spec 重验。
