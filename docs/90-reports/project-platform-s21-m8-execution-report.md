...
| PROJECT-PLATFORM-S21-M8-T07 | core-user | e2e-real-isolated | real | isolated | No | 管理者两层内进入设置管理首页、待办与嵌套自动化配置，普通成员不可见 |
...
| PROJECT-PLATFORM-S21-M8-T07 | 空间管理集中健康、待办、最近入口、危险操作和全部配置 | `ProjectSpaceSettingsPanel` 与 automation/metrics 两层嵌套导航 | route/tab 合同测试和 frontend lint/build PASS | real isolated：设置默认管理首页四分区可见，未就绪类型只出现在配置待办，自动化规则两层可达 | Done |
