...
Invoke-CountCheck `
    ...
    -Sql @"
select count(*)
from knowledge_item_permissions dp
join knowledge_base_items d
...
select count(*)
from resource_permissions rp
left join knowledge_base_items sd
...
with recursive kb_docs as (
    ...
    from knowledge_base_spaces k
    join knowledge_base_items d on d.workspace_id = k.workspace_id and d.id = k.root_item_id
    where k.deleted_at is null and d.deleted_at is null
    union all
    select kd.knowledge_base_id, c.workspace_id, c.id, c.parent_id
    from knowledge_base_items c
...
select count(*)
from search_index_entries si
left join knowledge_base_items d
...
"@ `
    -EvidenceSuffix "New content should be readable from knowledge_content_blocks; content-only rows require projection before block editor cutover."
...
select count(*)
from knowledge_content_blocks b
join knowledge_base_items d on d.workspace_id = b.workspace_id and d.id = b.item_id
...
where b.deleted_at is null
  ...
  and not exists (
      select 1
      from knowledge_content_blocks p
...
select count(*)
from (
    select workspace_id, item_id, parent_id, sort_order
    from knowledge_content_blocks
    where deleted_at is null
    group by workspace_id, item_id, parent_id, sort_order
    having count(*) > 1
) conflicts;
"@
...
Invoke-CountCheck `
    ...
    -Name "Retired knowledge content snapshot columns" `
    -Sql @"
select count(*)
...
select id, name, code, root_item_id, created_at
...
-- Keep search_index_entries and resource_permissions intact unless a separate audited cleanup is approved.

-- Optional block rollback preview: regenerate active blocks from document content for knowledge_base_items edited during cutover.
...
from knowledge_base_items d
left join knowledge_content_blocks b on b.workspace_id = d.workspace_id and b.item_id = d.id and b.deleted_at is null
where d.workspace_id = :'workspace_id'::uuid
