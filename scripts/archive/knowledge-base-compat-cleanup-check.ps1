...
$legacyPermissionWrites = Find-Text `
    ...
    -Include @("*.java") `
    -Pattern "(?i)\b(insert\s+into|update|delete\s+from)\s+knowledge_item_permissions\b"
if ($legacyPermissionWrites.Count -eq 0) {
    Add-Check "legacy-permission-table-write" "Runtime code does not write knowledge_item_permissions" "PASS" "No insert/update/delete against knowledge_item_permissions in server runtime code."
} else {
    Add-Check "legacy-permission-table-write" "Runtime code does not write knowledge_item_permissions" "FAIL" (Format-Evidence $legacyPermissionWrites)
}
...
Add-Decision "knowledge_base_items table" "Retain" "Content nodes, blocks, versions, comments and root/home pointers still depend on this storage model." "Delete only after a separate table migration, rollback plan and full route-final test pass."
Add-Decision "knowledge_item_permissions table" "Retain read/backfill only" "Runtime authorization uses resource_permissions; legacy rows may exist for migration and audit comparison." "Drop only after migration checks show zero dependencies and rollback no longer needs the table."
Add-Decision "knowledge_base_items.description/cover_url/default_permission_level on root nodes" "Retain deprecated shadow" "Local data may still contain shadow metadata; knowledge_base_spaces is authoritative." "Drop only after root shadow fields are cleared in production data and compatibility readers are removed."
Add-Decision "knowledge_base_items.knowledge_base and content_type='space'" "Retain deprecated compatibility flag" "Old root detection and deep-link compatibility still need it." "Drop only after all legacy space roots are represented solely by knowledge_base_spaces and fallback detection is removed."
