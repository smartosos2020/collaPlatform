...
$measurements.Add((Measure-Api "doc list" "GET" "/docs")) | Out-Null
...
if ($EnableMutatingChecks) {
    ...
    if ($docs.Count -gt 0) {
        ...
        $measurements.Add((Measure-Api "doc save: first document" "PATCH" "/docs/$($doc.document.id)" @{
...
$report = @(
    ...
    "",
    "| Name | Method | Path | Min ms | Avg ms | Max ms |",
    "| --- | --- | --- | ---: | ---: | ---: |"
)
foreach ($item in $measurements) {
    $report += "| $($item.Name) | $($item.Method) | `$($item.Path)` | $($item.MinMs) | $($item.AvgMs) | $($item.MaxMs) |"
}
