...
export type DocumentCollaborationSnapshot = {
  documentId: string
  title: string
  content: string
  serverClock: number
...
type UseDocumentCollaborationOptions = {
  ...
  canEdit: boolean
  enabled?: boolean
  onRemoteSnapshot: (snapshot: DocumentCollaborationSnapshot) => void
...
export function useDocumentCollaboration({
  ...
  const [status, setStatus] = useState<DocumentCollaborationStatus>('idle')
  ...
  useEffect(() => {
    const resetKey = `${documentId}:${versionNo}`
    ...
    setLastSavedAt(null)
    setError(null)
  }, [content, documentId, title, versionNo])
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      socket.onopen = () => {
        ...
        joinedRef.current = false
        sendCommand('document.join', {
          clientId,
          stateVector: String(serverClockRef.current),
        })
        sendCommand('document.snapshot.request', {
          clientId,
          stateVector: String(serverClockRef.current),
        })
      }
    ...
    return () => {
      ...
      sendCommand('document.leave', { clientId })
      socketRef.current?.close()
    }
    // documentId intentionally reconnects the socket so room membership remains single-doc.
  }, [accessToken, clientId, documentId, enabled])

  useEffect(() => {
    if (!enabled || !canEdit || !documentId) {
      return
    }
  ...
  }, [canEdit, content, documentId, enabled, title])

  const sendAwareness = useCallback((cursor: DocumentCursor) => {
    if (!enabled || !documentId || socketRef.current?.readyState !== WebSocket.OPEN) {
      return
    }
    sendCommand('document.awareness.update', {
      clientId,
      editing: canEdit,
      cursor,
    })
  }, [canEdit, clientId, documentId, enabled])
  ...
  function handleServerMessage(data: string) {
    ...
    if (event.objectType && event.objectType !== 'document') {
      return
    }
    const payload = (event.payload ?? {}) as Record<string, unknown>
    const eventDocumentId = String(payload.documentId ?? event.objectId ?? '')
    if (eventDocumentId && eventDocumentId !== documentId) {
      return
    }
    ...
    if (event.type === 'document.awareness.update') {
      updateClock(payload)
      updateOnlineUsers(payload)
  ...
  function updateOnlineUsers(payload: Record<string, unknown>) {
    ...
    setOnlineUsers(users.map(normalizeCollaborator).filter(Boolean) as DocumentCollaborator[])
  }
}
...
function normalizeCollaborator(value: unknown): DocumentCollaborator | null {
...
function normalizeCursor(value: unknown): DocumentCursor | null {
  if (!value || typeof value !== 'object') {
    return null
  }
