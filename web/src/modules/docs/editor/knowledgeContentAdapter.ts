...
export function blocksToTiptapDocument(blocks: DocumentBlock[]): JSONContent {
...
export function tiptapDocumentToBlockDrafts(document: JSONContent): DocumentBlockDraft[] {
...
export function blocksToMarkdown(blocks: Array<DocumentBlock | DocumentBlockDraft>) {
  return blocks.map(blockToMarkdown).filter((line) => line.length > 0).join('\n')
}

export function markdownToBlockDrafts(markdown: string): DocumentBlockDraft[] {
  const lines = markdown.replace(/\r\n/g, '\n').split('\n')
  const drafts: DocumentBlockDraft[] = []
...
function blockToTiptapNode(block: DocumentBlock): JSONContent | null {
...
function flattenTiptapNodes(nodes: JSONContent[]): DocumentBlockDraft[] {
...
function blockToMarkdown(block: DocumentBlock | DocumentBlockDraft) {
  const type = normalizeBlockType(block.blockType)
...
function markdownLineToDraft(line: string, sortOrder: number): DocumentBlockDraft {
...
function objectBlockToTiptap(block: DocumentBlock, type: string): JSONContent {
...
function objectCardNodeToBlockDraft(node: JSONContent): DocumentBlockDraft {
...
function fileCardNodeToBlockDraft(node: JSONContent): DocumentBlockDraft {
...
function objectDirectiveToBlockDraft(attrs: Record<string, string>, sortOrder: number): DocumentBlockDraft {
...
function fileDirectiveToBlockDraft(attrs: Record<string, string>, sortOrder: number): DocumentBlockDraft {
...
function embedBlockToDirective(type: string, content: string, block: DocumentBlock | DocumentBlockDraft) {
...
function blockTypeForObjectType(objectType: string): DocumentBlockDraft['blockType'] {
...
function tableBlockToTiptap(block: DocumentBlock): JSONContent {
