...
export function KnowledgeContentEditorCore({
  ...
  const [fullEditorState, setFullEditorState] = useState(() => ({
    ...
    loaded: !contentStats.largeDocument,
  }))
  ...
  const uploadFileIntoEditor = useCallback(
    async (targetEditor: Editor | null, file: File, preferredKind?: 'image' | 'file') => {
      ...
      setUploading(true)
      try {
        const metadata = await uploadFileForTarget(file, 'knowledge_content', itemId)
    ...
    <section className="doc-editor-shell">
      ...
      {contentStats.largeDocument && !fullEditorLoaded ? (
        <Alert
          ...
          type="info"
          message="大内容预览模式"
          description={`当前知识内容约 ${contentStats.blockCount} 块、${contentStats.embedCount} 个嵌入。已先渲染前 ${LARGE_DOCUMENT_PREVIEW_BLOCKS} 块，避免移动端或低性能设备卡顿。`}
