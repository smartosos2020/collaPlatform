  ...
  test('isolated roles use server-projected type configuration and execution summaries @smoke', async ({ browser, page, request }) => {
    ...
    try {
      ...
      for (const identity of [member, guest]) {
        ...
        await expect(executionPage.getByLabel('工作入口')).toContainText(`${suffix}_bug`)
        await expect(executionPage.getByLabel('工作入口')).not.toContainText(`${suffix}_requirement_copy`)
