...
test.describe('PROJECT-PLATFORM-S03-M4 preset and compatibility UI', () => {
  test('new and existing spaces expose protected presets while legacy projects remain usable @smoke', async ({ page, request }) => {
    ...
    try {
      ...
      const projectTypeOption = page.getByRole('option', { name: /项目.*project/ })
      await expect(projectTypeOption.locator('.work-item-type-status-dot')).toBeVisible()
      await expect(projectTypeOption.locator('.status-badge')).toHaveCount(0)
      await projectTypeOption.click()
      await expect(page.getByText('来源：研发预置目录 development-v1')).toBeVisible()
      const typeInformation = page.locator('.work-item-type-detail')
      const typeInformationHeader = typeInformation.locator('.work-item-type-detail-header')
      await expect(typeInformationHeader.locator('.work-item-type-glyph')).toHaveCount(0)
      ...
      await expect(typeInformation.locator('.work-item-type-protection-note')).toBeVisible()
      await expect(typeInformation.locator('.ant-alert')).toHaveCount(0)
      ...
      const sourceHandle = page.getByRole('button', { name: '拖拽排序 项目' })
      const targetRow = page.getByRole('option', { name: /需求.*requirement/ }).locator('..')
      const sourceBox = await sourceHandle.boundingBox()
      const targetBox = await targetRow.boundingBox()
      if (!sourceBox || !targetBox) throw new Error('工作项类型拖拽手柄或目标行不可见')
      await page.mouse.move(sourceBox.x + sourceBox.width / 2, sourceBox.y + sourceBox.height / 2)
      await page.mouse.down()
      await page.mouse.move(targetBox.x + targetBox.width / 2, targetBox.y + targetBox.height * 0.75, { steps: 6 })
      await page.mouse.up()
      expect((await reorderResponse).ok()).toBeTruthy()
      const expectedOrder = initialOrder.filter((typeKey) => typeKey !== 'project')
      expectedOrder.splice(expectedOrder.indexOf('requirement') + 1, 0, 'project')
      await expect.poll(() => typeKeys(page)).toEqual(expectedOrder)
      await page.getByRole('button', { name: '拖拽排序 项目' }).click()
      await expect(page.getByLabel('工作项类型列表').getByRole('status')).toHaveText('未调整顺序')
      await expect(page.getByRole('button', { name: '下移 项目' })).toHaveCount(0)
