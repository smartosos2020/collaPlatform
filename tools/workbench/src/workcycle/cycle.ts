...
async function verify(options: WorkCycleOptions): Promise<void> {
  ...
  if (hasSpecs) {
    ...
    const isolatedS12Route = evidenceKind === 'real'
      && evidenceEnvironment === 'isolated'
      && browserSpecs.length === 1
      && /^project-platform-s12-[a-z0-9-]+\.spec\.ts$/.test(
        browserSpecs[0].replaceAll('\\', '/').split('/').at(-1) ?? '',
      )
    const isolatedS13Route = evidenceKind === 'real'
      && evidenceEnvironment === 'isolated'
      && browserSpecs.length === 1
      && /^project-platform-s13-[a-z0-9-]+\.spec\.ts$/.test(
        browserSpecs[0].replaceAll('\\', '/').split('/').at(-1) ?? '',
      )
    const isolatedS14Route = evidenceKind === 'real'
      && evidenceEnvironment === 'isolated'
      && browserSpecs.length === 1
      && /^project-platform-s14-[a-z0-9-]+\.spec\.ts$/.test(
        browserSpecs[0].replaceAll('\\', '/').split('/').at(-1) ?? '',
      )
    const isolatedS15Route = evidenceKind === 'real'
      && evidenceEnvironment === 'isolated'
      && browserSpecs.length === 1
      && /^project-platform-s15-[a-z0-9-]+\.spec\.ts$/.test(
        browserSpecs[0].replaceAll('\\', '/').split('/').at(-1) ?? '',
      )
    const isolatedS16Route = evidenceKind === 'real'
      && evidenceEnvironment === 'isolated'
      && browserSpecs.length === 1
    ...
    const isolatedS18Route = evidenceKind === 'real'
      && evidenceEnvironment === 'isolated'
      && browserSpecs.length === 1
      && /^project-platform-s18-[a-z0-9-]+\.spec\.ts$/.test(
        browserSpecs[0].replaceAll('\\', '/').split('/').at(-1) ?? '',
      )
    const isolatedS19Route = evidenceKind === 'real'
      && evidenceEnvironment === 'isolated'
      && browserSpecs.length === 1
      && /^project-platform-s19-[a-z0-9-]+\.spec\.ts$/.test(
        browserSpecs[0].replaceAll('\\', '/').split('/').at(-1) ?? '',
      )
    const isolatedS21Route = evidenceKind === 'real'
      && evidenceEnvironment === 'isolated'
      && browserSpecs.length === 1
      && /^project-platform-s21-[a-z0-9-]+\.spec\.ts$/.test(
        browserSpecs[0].replaceAll('\\', '/').split('/').at(-1) ?? '',
      )
    ...
    } else if (isolatedS12Route) {
      const { isolatedProjectPlatformS12Smoke } = await import('../browser/smoke.js')
      const spec = browserSpecs[0].replaceAll('\\', '/').split('/').at(-1)!
      await isolatedProjectPlatformS12Smoke(repositoryRoot, spec)
      command = `pnpm workbench browser smoke-project-platform-s12-isolated --spec ${spec}`
      output = `PROJECT-PLATFORM-S12 isolated browser smoke passed: ${spec}.\n`
    } else if (isolatedS13Route) {
      const { isolatedProjectPlatformS13Smoke } = await import('../browser/smoke.js')
      const spec = browserSpecs[0].replaceAll('\\', '/').split('/').at(-1)!
      await isolatedProjectPlatformS13Smoke(repositoryRoot, spec)
      command = `pnpm workbench browser smoke-project-platform-s13-isolated --spec ${spec}`
      output = `PROJECT-PLATFORM-S13 isolated browser smoke passed: ${spec}.\n`
    } else if (isolatedS14Route) {
      const { isolatedProjectPlatformS14Smoke } = await import('../browser/smoke.js')
      const spec = browserSpecs[0].replaceAll('\\', '/').split('/').at(-1)!
      await isolatedProjectPlatformS14Smoke(repositoryRoot, spec)
      command = `pnpm workbench browser smoke-project-platform-s14-isolated --spec ${spec}`
      output = `PROJECT-PLATFORM-S14 isolated browser smoke passed: ${spec}.\n`
    } else if (isolatedS15Route) {
      const { isolatedProjectPlatformS15Smoke } = await import('../browser/smoke.js')
      const spec = browserSpecs[0].replaceAll('\\', '/').split('/').at(-1)!
      await isolatedProjectPlatformS15Smoke(repositoryRoot, spec)
      command = `pnpm workbench browser smoke-project-platform-s15-isolated --spec ${spec}`
      output = `PROJECT-PLATFORM-S15 isolated browser smoke passed: ${spec}.\n`
    } else if (isolatedS16Route) {
      const { isolatedProjectPlatformS16Smoke } = await import('../browser/smoke.js')
      const spec = browserSpecs[0].replaceAll('\\', '/').split('/').at(-1)!
    ...
    } else if (isolatedS18Route) {
      const { isolatedProjectPlatformS18Smoke } = await import('../browser/smoke.js')
      const spec = browserSpecs[0].replaceAll('\\', '/').split('/').at(-1)!
      await isolatedProjectPlatformS18Smoke(repositoryRoot, spec)
      command = `pnpm workbench browser smoke-project-platform-s18-isolated --spec ${spec}`
      output = `PROJECT-PLATFORM-S18 isolated browser smoke passed: ${spec}.\n`
    } else if (isolatedS19Route) {
      const { isolatedProjectPlatformS19Smoke } = await import('../browser/smoke.js')
      const spec = browserSpecs[0].replaceAll('\\', '/').split('/').at(-1)!
      await isolatedProjectPlatformS19Smoke(repositoryRoot, spec)
      command = `pnpm workbench browser smoke-project-platform-s19-isolated --spec ${spec}`
      output = `PROJECT-PLATFORM-S19 isolated browser smoke passed: ${spec}.\n`
    } else if (isolatedS21Route) {
      const { isolatedProjectPlatformS21Smoke } = await import('../browser/smoke.js')
      const spec = browserSpecs[0].replaceAll('\\', '/').split('/').at(-1)!
      await isolatedProjectPlatformS21Smoke(repositoryRoot, spec)
      command = `pnpm workbench browser smoke-project-platform-s21-isolated --spec ${spec}`
      output = `PROJECT-PLATFORM-S21 isolated browser smoke passed: ${spec}.\n`
    } else {
