...
export async function healthCheck(root: string, options: HealthOptions): Promise<string> {
  ...
  try {
    ...
    if (options.metricsBaseUrl) {
      const actuator = await (await checkedGet(`${metrics}/actuator/health`)).json() as any
      if (actuator.status !== 'UP') throw new Error('Actuator health is not UP')
      results.push('Actuator health is UP on the explicit internal metrics endpoint')
    }
    if (options.requirePrometheus) {
      if (!options.metricsBaseUrl) throw new Error('Prometheus verification requires an explicit internal --metrics-base-url')
      const text = await (await checkedGet(`${metrics}/actuator/prometheus`)).text()
      if (!/^jvm_/m.test(text)) throw new Error('Prometheus output has no JVM metrics')
      results.push('Prometheus exposes JVM metrics')
    }
    if (options.requireLogCorrelation) {
      ...
      for (let attempt = 0; attempt < 10 && !matched; attempt += 1) {
        matched = compose(paths, ['logs', '--since', '2m', 'api-a', 'api-b']).includes(requestId)
        if (!matched) await new Promise((resolve) => setTimeout(resolve, 500))
      }
      if (!matched) throw new Error(`Request id not found in API logs: ${requestId}`)
      results.push('request id is present in API logs')
    }
    passed = true
  } finally {
