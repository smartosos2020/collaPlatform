...
export async function isolatedProjectPlatformS20Smoke(
  root: string,
  spec: string,
  databasePort = 5432,
  apiPort = 18200,
  ...
  if (!/^project-platform-s20-[a-z0-9-]+\.spec\.ts$/.test(spec)) {
    throw new Error(`Unsupported PROJECT-PLATFORM-S20 browser spec: ${spec}`)
  }
  return isolatedRouteSmoke(
    root,
    spec,
