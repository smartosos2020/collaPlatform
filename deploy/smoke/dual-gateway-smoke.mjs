...
function openSocket(token) {
  return new Promise((resolve, reject) => {
    ...
    socket.addEventListener("message", (event) => {
      try {
        if (JSON.parse(String(event.data)).type !== "connection.ready") return;
        clearTimeout(timer);
        resolve(socket);
      } catch {
...
function nginxLogs() {
  return execFileSync("docker", ["logs", "--since", startedAt, containerId("nginx")], {
    encoding: "utf8",
    stdio: "pipe",
  });
}
...
function assertDistribution(logs) {
  for (const service of gatewayServices) {
    ...
    if (!logs.includes(address)) throw new Error(`Nginx WebSocket distribution did not reach ${service} at ${address}`);
  }
}
...
try {
  ...
  await Promise.all(distributedSockets.map(closeSocket));
  await sleep(500);
  assertDistribution(nginxLogs());

  stopService("event-gateway-a");
  const gatewayBFallback = await openSocket(token);
  await closeSocket(gatewayBFallback);
  ...
  await closeSocket(gatewayAFallback);
